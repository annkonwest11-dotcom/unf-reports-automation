...
def build_files(only=None, outdir=None):
    ...
    reports = [
        ...
        dict(who="Валерия Папоян", fname="Взаиморасчеты ПАПОЯН ВАЛЕРИЯ",
             descr="Валерия Папоян — клиенты поиска",
             pred=lambda f, g, h: f == "Валерия Папоян",
             bottom=[f"ФАКТ ЗА {month_label} — оплаты всех клиентов (в расчёт ЗП)",
                     ("Новые клиенты — факт", kpi("Валерия Папоян", "Оплаты (новые клиенты)")),
                     ("% выполнения плана по новым",
                      kpi("Валерия Папоян", "% выполнения плана"))]),
