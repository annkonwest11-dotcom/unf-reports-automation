...
# Осн. смены — строки 5-11 (Дарья/Ксения/Папоян/Алена/Ия/Влада/Анна),
# подработки — строки 16-19 (Алена/Ия/Влада/Анна). Раскладка после добавления
# Ии Шарашанидзе (2026-07-13); диапазоны покрывают ровно строки сотрудников.
_SHIFT_DAY_RANGES = ('E5:AI11', 'E16:AI19')
...
class SheetsClient:
    ...
    def archive_month(self) -> str:
        ...
        try:
            ...
            def _grid(sid, r0, r1, c0, c1):
                return {"sheetId": sid, "startRowIndex": r0, "endRowIndex": r1,
                        "startColumnIndex": c0, "endColumnIndex": c1}
            # Осн. смены — строки 5-11 (0-инд 4..11), подработки — строки 16-19
            # (0-инд 15..19); колонки E(4)…AI(35). Совпадает с _SHIFT_DAY_RANGES.
            sh.batch_update({"requests": [
                {"copyPaste": {"source": _grid(ws_next.id, 4, 11, 4, 35),
                               "destination": _grid(ws_smeny.id, 4, 11, 4, 35),
                               "pasteType": "PASTE_VALUES", "pasteOrientation": "NORMAL"}},
                {"copyPaste": {"source": _grid(ws_next.id, 15, 19, 4, 35),
                               "destination": _grid(ws_smeny.id, 15, 19, 4, 35),
                               "pasteType": "PASTE_VALUES", "pasteOrientation": "NORMAL"}},
            ]})
        ...
        # добавления Ии Шарашанидзе (2026-07-13). Формулы НЕ трогаем:
        # конкурс Ксении/Папояна (лист КОНКУРС) и C=E у Влады/РОП.
        ws_sv = sh.worksheet('СВОДНАЯ_ЗП')
        manual_ranges = [
            'C35:C37',            # Дарья: конкурс, доп.премия, комментарий
            'C48', 'C50:C51',     # Ксения: бонус-сторис, доп.премия, комментарий
            'C63:C64',            # Папоян: доп.премия, комментарий
            'C74:C75',            # Алена: доп.премия, комментарий
            'C83:C84',            # Ия: доп.премия, комментарий
            'E93', 'E98', 'E102', # РОП: факты (новые продажи / оборот / поступления)
            'E106:E107',          # РОП: доп.премия, комментарий
            'E116',               # Влада: % контента
            'E119:F122',          # Влада: найм (кол-во + ФИО)
            'E125',               # Влада: премия за проекты
            'E129:E130',          # Влада: доп.премия, комментарий
            'C137:E144',          # Таблица выплат: авансы / наличные / ЗП на карту
        ]
