...
# Осн. смены — строки 5-10 (Дарья/Ксения/Папоян/Алена/Влада/Анна),
# подработки — строки 15-17 (Алена/Влада/Анна). Раскладка после удаления
# Ии Шарашанидзе (2026-07-23); диапазоны покрывают ровно строки сотрудников.
_SHIFT_DAY_RANGES = ('E5:AI10', 'E15:AI17')
...
class SheetsClient:
    ...
    def archive_month(self) -> str:
        ...
        try:
            ...
            def _grid(sid, r0, r1, c0, c1):
                return {"sheetId": sid, "startRowIndex": r0, "endRowIndex": r1,
                        "startColumnIndex": c0, "endColumnIndex": c1}
            # Осн. смены — строки 5-10 (0-инд 4..10), подработки — строки 15-17
            # (0-инд 14..17); колонки E(4)…AI(35). Совпадает с _SHIFT_DAY_RANGES.
            sh.batch_update({"requests": [
                {"copyPaste": {"source": _grid(ws_next.id, 4, 10, 4, 35),
                               "destination": _grid(ws_smeny.id, 4, 10, 4, 35),
                               "pasteType": "PASTE_VALUES", "pasteOrientation": "NORMAL"}},
                {"copyPaste": {"source": _grid(ws_next.id, 14, 17, 4, 35),
                               "destination": _grid(ws_smeny.id, 14, 17, 4, 35),
                               "pasteType": "PASTE_VALUES", "pasteOrientation": "NORMAL"}},
            ]})
        ...
        # удаления Ии Шарашанидзе (2026-07-23). Формулы НЕ трогаем:
        # конкурс Ксении C48/Папояна C61 (лист КОНКУРС) и C=E у Влады/РОП.
        ws_sv = sh.worksheet('СВОДНАЯ_ЗП')
        manual_ranges = [
            'C34:C36',            # Дарья: конкурс, доп.премия, комментарий
            'C47', 'C49:C50',     # Ксения: бонус-сторис, доп.премия, комментарий
            'C62:C63',            # Папоян: доп.премия, комментарий
            'C73:C74',            # Алена: доп.премия, комментарий
            'E88', 'E92',         # РОП: факты (оборот-авто / поступления) — перезапишет синк 10:35
            # ★ E83 (факт новых продаж) НЕ чистим — с 2026-08-04 это ФОРМУЛА
            # по листу НОВЫЕ_КЛИЕНТЫ, она сама пересчитается на новый месяц.
            # Чистка её убивала (проверено на закрытии июля 2026).
            'E96:E97',            # РОП: доп.премия, комментарий
            'E106',               # Влада: % контента
            'E107:G107',          # Влада: клиенты SMM (A/B/C)
            'E109:F113',          # Влада: найм (кол-во + ФИО) + бонус за план найма
            'E115:F115',          # Влада: премия за проекты + название
            'E119:E120',          # Влада: доп.премия, комментарий
            'C127:E133',          # Таблица выплат: авансы / наличные / ЗП на карту
        ]
