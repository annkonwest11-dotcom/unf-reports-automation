...
# Имя из заголовка колонки KPI (после kpi_key) → (ФИО менеджера, тип).
# ★23.09.2026: ключи — ОДНО ИМЯ, без слова «Менеджер». Анна пишет заголовок то
# «Менеджер Дарья» (лист «июль-август»), то просто «Дарья» (лист «август-сентябрь»),
# а сравнение было точным — за сентябрь распозналась бы ОДНА колонка «Анна», и у
# всех менеджеров поиска % плана, фикс-бонус и ставка ушли бы в ноль. Теперь
# префикс «менеджер» срезается, «ё» приводится к «е» — см. kpi_key().
KPI_MANAGERS = {
    "дарья": ("Дарья Вольнова", "поиск"),
    "алена": ("Алена Черкашина", "сопровождение"),
    "ксения": ("Ксения Наныкина", "поиск"),
    # Лилия Сулименко — менеджер поиска с августа 2026. Колонку в KPI-таблице
    # заводит Анна; пока её нет, строка просто не встречается.
    "лилия": ("Лилия Сулименко", "поиск"),
    # Стажёры-менеджеры поиска с 01.09.2026 — колонки заводит Анна.
    "софия": ("София Хакимова", "поиск"),
    "виолетта": ("Виолетта Караханова", "поиск"),
    # Валерия Папоян уволена 11.08.2026 — её колонка в KPI больше не разбирается,
    # клиенты из СПРАВОЧНИКА переведены на «Прямой клиент» (решение Анны).
    "анна": ("Анна Кононенко (РОП)", "роп"),
    # Анна 04.08: продажи из колонки Влады пока вешаем на «Прямой клиент» +
    # сопровождение Валерия Абрамова (парковка, своего блока в СВОДНОЙ у неё нет).
    # ⚠️Абрамова уволена, её клиенты 23.09.2026 переведены на Алёну; в листе
    # «август-сентябрь» колонки «Влада» уже нет, так что парковка не срабатывает.
    "влада": ("Валерия Абрамова", "сопровождение"),
}


def kpi_key(header):
    """Заголовок колонки KPI → ключ KPI_MANAGERS («Менеджер Алёна» → «алена»)."""
    h = (header or "").strip().lower().replace("ё", "е")
    return re.sub(r"^менеджер\s+", "", h).strip()
...
def sync_novye(dry_run=False):
    ...
    # колонки менеджеров
    mgr_cols = {i: KPI_MANAGERS[kpi_key(headers[i])]
                for i in range(len(headers)) if kpi_key(headers[i]) in KPI_MANAGERS}
