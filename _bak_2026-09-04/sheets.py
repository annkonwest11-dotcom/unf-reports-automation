...
class SheetsClient:
    ...
    def archive_month(self) -> str:
        ...
        # добавления Лилии Сулименко (2026-08-14). Формулы НЕ трогаем:
        # конкурс (лист КОНКУРС) у всех менеджеров поиска и C=E у Влады/РОП.
        ws_sv = sh.worksheet('СВОДНАЯ_ЗП')
        manual_ranges = [
            # ★ C33 (конкурс Дарьи) НЕ чистим — это ФОРМУЛА по листу КОНКУРС,
            # как C47 у Ксении и C60 у Лилии. Чистка её убивала: после закрытия
            # июля 2026 бонус конкурса у Дарьи перестал считаться (нашлось 14.08).
            'C34:C35',            # Дарья: доп.премия, комментарий
            'C46', 'C48:C49',     # Ксения: бонус-сторис, доп.премия, комментарий
            'C61:C62',            # Лилия: доп.премия, комментарий
            'C72:C73',            # Алена: доп.премия, комментарий
            'E87', 'E91',         # РОП: факты (оборот-авто / поступления) — перезапишет синк 10:35
            # ★ E82 (факт новых продаж) НЕ чистим — с 2026-08-04 это ФОРМУЛА
            # по листу НОВЫЕ_КЛИЕНТЫ, она сама пересчитается на новый месяц.
            # Чистка её убивала (проверено на закрытии июля 2026).
            'E95:E96',            # РОП: доп.премия, комментарий
            'E105',               # Влада: % контента
            'E106:G106',          # Влада: клиенты SMM (A/B/C)
            'E108:F112',          # Влада: найм (кол-во + ФИО) + бонус за план найма
            'E114:F114',          # Влада: премия за проекты + название
            'E118:E119',          # Влада: доп.премия, комментарий
            'C126:E131',          # Таблица выплат: авансы / наличные / ЗП на карту
        ]
