...
ALIASES = {
    ...
    # ★ Разбор 05.09.2026 «кого нет в справочнике»: карточки, где 1С и таблица
    # разошлись порядком слов или хвостом. Суммы в листы ДАННЫЕ ложились верно
    # (там уже канон), а ритм/добивка ответственного не находили — задачи уходили
    # в общий котёл. Клиенты живые, обороты от 5 до 154 тыс.
    "УГОЛЕК ООО (АНИЧА)": "АНИЧА ООО (Уголёк)",
    "Loft Holl ООО Дабл Топ Кейтеринг смотреть факт адрес":
        "Loft Holl ООО Дабл Топ Кейтеринг",
    "Гайтукиев С.Р. ИП (ЗДРАСЬТЕ БИСТРО)": "ЗДРАСЬТЕ БИСТРО (ИП Гайтукиев С.Р.)",
    "The Caters ИП ДМИТРИЕВ ЕВГЕНИЙ НИКОЛАЕВИЧ зе катерс":
        "The Caters (ИП ДМИТРИЕВ ЕВГЕНИЙ НИКОЛАЕВИЧ) в 1С",
    "ООО ВИНОВНИКИ ( РУВИ)": "ВИНОВНИКИ (ООО РУВИ)",
    "ПИЦЦА 25 (ИП МАНУЙЛЕНКО В.Ю.)": "ПИЦЦА 25 (ИП МАНУЙЛЕНКО)",
    "NAIOLI (ООО НАЙОЛИ) ЭДО СЧЕТ": "NAIOLI (ООО НАЙОЛИ)",
    "МУН ЛАУНЖ (ООО МУНТЕРРА ГРУПП)": "Мун Лаунж ООО ( МУНТЕРРА ГРУПП)",
    "КРАПИВА (ИП Викторов Анатолий Александрович)":
        "Викторов Анатолий Александрович (ООО Крапива) 1с",
    "Bluefin  М-ГРУПП нал": "Bluefin ООО М-ГРУПП",
    "КОФЕЙНЯ У ПРУДА ООО ЭДО": "КОФЕЙНЯ У ПРУДА ООО",
    "БРЕЙГЕЛЬ ООО (Семпре)": "«БРЕЙГЕЛЬ» ООО",
    "СПОРТКОРТ ООО ЭДО адрес дост менять": "СПОРТКОРТ ООО ЭДО адрес дост менять",

    # ★ Разбор 09.09.2026: у Ахмедова в 1С ДВА ДВОЙНИКА — долг 2 762 остался на
    # карточке «… в 1С», а оплата за август прошла по карточке без метки. В лист
    # суммы ложились верно (plan_updates добирал вторую карточку мягким матчем),
    # но zp_files.balance_for сначала пробует ТОЧНОЕ имя, попадал на карточку с
    # меткой и получал ноль — файл Алёны недобирал 2 762 против листа.
    # Пара сводит обе карточки в один канон, load_balances их складывает:
    # нач 2 762 + оплата 2 762 = кон 0, как и должно быть у клиента.
    "ПАНОРАМА (ИП Ахмедов Эльбрус Нураддин Оглы)":
        "ПАНОРАМА (ИП Ахмедов Эльбрус Нураддин Оглы) в 1С",
...
_NOISE_WORDS = {
    ...
    # ★05.09.2026: из-за них одна и та же карточка не находилась и оплаты
    # не попадали в лист. «сбис» — система ЭДО, как dsbx: «ТЕРРИН (ООО
    # ГАСТРОКЛУБ) ЭДО сбис» ≠ «ТЕРРИН (ООО ГАСТРОКЛУБ)» → потеряно 35 000 ₽.
    "сбис", "сбиc", "диадок", "контур", "xml", "хml",
    "ликвидирован", "ликвидирована", "ликв", "закрыт", "закрыта",
    "проверять", "проверить", "смотреть", "факт", "сразу", "быстро",
}
# Пометка о переводе клиента между базами. Пишут её по-разному:
#   «… с 10.08.26 на ПЕРФИЛЬЕВ»   — перевод августа;
#   «… НА ИП ВОЛОДИХИНА», «… НА ИП ВОЛ» — перевод на третье ИП (сентябрь 2026),
#     уже без даты и часто обрезанное.
# ★Второй формат не распознавался, и клиент выглядел новой карточкой: беби-вычет
# терял SAVVA и ЛИМОНЧИНО, ритм считал их «пропавшими» (разбор 05.09.2026).
# Срезаем только когда после «на» стоит владелец базы, иначе пострадают честные
# имена вроде «КОФЕЙНЯ НА ПАТРИКАХ».
_BASE_OWNERS = r"(?:перфильев\w*|губарев\w*|володихин\w*|вол)"
_TRANSFER_TAIL = re.compile(
    rf"\s+(?:с\s*\d{{2}}\.\d{{2}}\.\d{{2,4}}\s*)?на\s+(?:ип\s+)?{_BASE_OWNERS}\s*$", re.I)
...
def name_tokens(s):
    ...
    ↔ «РУБИН ООО (ЖАН ЖАК)»), разошлись в ё/е («ВАСИЛЁК» ↔ «ВАСИЛЕК») или в
    дефисе («БУТИК-БАР ДЖАННЕТ» ↔ «БУТИК БАР ДЖАННЕТ») — дефис тоже разделитель.
    """
...
def same_client(t1, t2):
    """Наборы различающих слов описывают одного контрагента.

    Требуем полного совпадения. Вложенности (как в soft_lookup) здесь мало:
    лишнее слово у соседа — это чаще всего НАЗВАНИЕ ТОЧКИ, и тогда склейка
    объединяет разные заведения сети. Проверено на живых данных: «ШЕСТНАДЦАТЬ
    ТОНН КЕЙТЕРИНГ» ⊂ «ШЕСТНАДЦАТЬ ТОНН ПРЕСНЯ» — точки разные. Переезды между
    базами при этом ловятся полностью: имя то же, отличается только хвост
    «с ДД.ММ.ГГ на ФАМИЛИЮ», а его срезает name_tokens. Карточка без своих слов
    не склеивается ни с кем.
    """
    return bool(t1) and t1 == t2


def _long_numbers(name):
    """Телефоны и прочие длинные номера из названия карточки.

    Короткие числа (даты вроде «с 10.08.26», «А20») не в счёт — они не различают
    контрагентов. А вот разные телефоны различают: «коллеги (НАЛ) 89684708830» и
    «+79299588161 коллеги» — по словам одно и то же, по номеру это разные люди.
    """
...
def group_same_client(names):
    ...
    Если у карточки больше одного кандидата, не склеиваем: лучше лишняя строка,
    чем перепутанные клиенты.
    """
    names = list(names)
    toks = {n: key_tokens(n) for n in names}
    nums = {n: _long_numbers(n) for n in names}
...
def actual_name(names):
    """Из карточек одного клиента — та, что в ходу: без пометки о переводе."""
    return min(names, key=lambda n: (bool(_TRANSFER_TAIL.search(n)), -len(n)))
