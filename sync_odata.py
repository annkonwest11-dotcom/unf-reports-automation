...
ALIASES = {
    ...
    # ★05.09.2026: клиент переехал на другую карточку, но общая склейка их не
    # берёт — с 17.08 запрещено объединять по вложенности имён (иначе
    # «ШЕСТНАДЦАТЬ ТОНН КЕЙТЕРИНГ» слипнется с «…ПРЕСНЯ»). Здесь связываем
    # точечно: старая карточка в Перфильеве замолчала 23.07, новая в Губареве
    # с 11.08 заказывает регулярно — Анна и в отчёте считает их вместе.
    "Angel Cakes": "Angel Cakes (ИП ТЮТЬКОВА А А) ангел кейкс СЧЕТ",
