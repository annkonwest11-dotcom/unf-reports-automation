...
«ОТЧЕТЫ ПО ПРОДАЖАМ» (лист на месяц, последний — «июль-август»):
  ...
  • ВЕРНУЛИ  — заказ после молчания ≥28 дней, которое ВДВОЕ длиннее личного ритма
               клиента (медианы интервалов между его заказами). Голый порог не
               годится: клиент, берущий раз в месяц (ОТДЫХ Щелково, КОФЕ В ПАРКЕ,
               СТЕЙК ИТ ИЗИ Монтонико), «возвращался» бы каждый месяц;
...
RETURN_GAP = 28        # минимальное молчание, чтобы вообще считаться возвратом
RETURN_RATIO = 2.0     # во столько раз молчание должно превышать личный ритм
LONG_SILENCE = 60      # молчание, которое считаем возвратом даже без истории ритма
...
def personal_rhythm(items, until, lookback=180):
    ...
    ★Сам `until` (последний заказ перед молчанием) входит в выборку: без него
    теряется последний интервал и ритм занижается — ОТДЫХ Щелково с ритмом 17,5
    превращался в клиента с ритмом 14 и ложно попадал в «вернувшиеся».
    """
...
def classify(hist, ym):
    ...
    for cli, items in hist.items():
        ...
        if first_ever >= m_start:
            ...
            rhythm = personal_rhythm(items, prev)
            returned = gap >= RETURN_GAP and (
                (rhythm and gap >= RETURN_RATIO * rhythm) or (not rhythm and gap >= LONG_SILENCE))
            if not returned:
                continue
