...
class NotAnInvoice(ValueError):
    """Документ есть, но это не товарная накладная (обычно счёт на оплату)."""


def doc_kind(text):
    """Что за документ: «накладная» | «счёт» | «неизвестно».

    Юшин шлёт в группу и накладные, и счета на оплату — в отчёт идут только
    накладные (правило Анны 18.08). Порядок проверок важен: в накладной бывает
    строка «Основание: Счет на оплату № …», поэтому сперва ищем накладную и
    только потом счёт, иначе накладная опозналась бы как счёт.
    """
    # ★сперва lower, потом ё→е: в PDF заголовок часто заглавными («СЧЁТ НА ОПЛАТУ»),
    # и замена до lower не трогала заглавную Ё
    t = re.sub(r"\s+", " ", str(text or "")).lower().replace("ё", "е")
    if "товарная накладная" in t or "торг-12" in t or "торг 12" in t:
        return "накладная"
    if "счет на оплату" in t or "счет-оферта" in t:
        return "счет"
    return "неизвестно"
...
def parse(path):
    ...
    Если это не накладная (счёт на оплату), поднимаем NotAnInvoice — вызывающий
    решает, промолчать или ответить.
    """
    text = extract_text(path)
    kind = doc_kind(text)
    if kind != "накладная":
        raise NotAnInvoice(
            "это счёт на оплату, не накладная" if kind == "счет"
            else "не похоже на товарную накладную")
