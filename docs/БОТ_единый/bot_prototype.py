...
# ─── KPI ВЛАДЫ (4 блока, ручной ввод за месяц) ───
# Структура: {"Влада": {
#   "hr": 0,        # БЛОК HR: сумма бонусов за приём (20к за план, 5к за 7 дней, 20к за месяц, 10к за производ, 20к за водител)
#   "smm_salary": 25000,  # БЛОК SMM: оклад 25к за задачи
#   "smm_bonus": 0,  # БЛОК SMM: бонус 90%+ плана контента (+5к)
#   "smm_clients": 0,  # БЛОК SMM: сумма за привёденных клиентов (А:7к, В:4к, С:2к)
#   "projects": 0,   # БЛОК ПРОЕКТЫ: премии за сроки (индивидуально, минус за просрочку)
#   "logistics": 10000,  # БЛОК ЛОГИСТИКА: бонус за администрирование (10к при 0 ошибок, -5% за каждую ошибку/пропуск)
# }}
# ОТВЕТ АННЫ: Логистика вручную финальное число (не считаем ошибки).

# ─── KPI РОП (Анна) (3 блока бонусов + премия, ручной ввод за месяц) ───
# Структура: {"Анна Кононенко": {
#   "plan_new_sales": 0,      # БЛОК 2: план по новым продажам (₽), ручной ввод 1-го числа
#   "fact_new_sales": 0,      # БЛОК 2: факт по новым продажам (₽), из таблицы/вручную
#   "plan_turnover": 0,       # БЛОК 3: план по общему обороту (₽), ручной ввод 1-го числа
#   "fact_turnover": 0,       # БЛОК 3: факт по общему обороту (₽), из 1С
#   "plan_payments": 0,       # БЛОК 4: план по поступлениям оплат (₽), ручной ввод 1-го числа
#   "fact_payments": 0,       # БЛОК 4: факт по поступлениям оплат (₽), из таблицы/1С
# }}
# Расчет для каждого блока (2, 3, 4):
#   - Если факт >= 70% плана:
#     - Если факт >= 100% плана: бонус = факт * 1%
#     - Если факт >= 70% плана: бонус = факт * 0.7%
#   - Если факт >= 115% плана (только блок 2): добавить +10 000 ₽


def kpi_rop_block(plan: float, fact: float) -> float:
    """РОП: расчет одного блока бонуса (2, 3 или 4). Возвращает размер бонуса (₽)."""
    if plan <= 0:
        return 0.0
    pct = fact / plan * 100
    if pct < 70:
        return 0.0
    if pct >= 100:
        return round(fact * 0.01)  # 1% от факта при 100%+
    else:  # 70% <= pct < 100
        return round(fact * 0.007)  # 0.7% от факта при 70-99%


def kpi_rop_premium(plan: float, fact: float) -> float:
    """РОП: премия за перевыполнение (только блок 2 - новые продажи). Возвращает 10000 или 0."""
    if plan <= 0:
        return 0.0
    pct = fact / plan * 100
    return 10000.0 if pct >= 115 else 0.0
...
class SalaryRow:
    ...
    base_pay: float = 0.0
    side_pay: float = 0.0
    plan_pct: float = 0.0        # % выполнения плана (для KPI поиска)
    kpi_fix: float = 0.0         # фикс-бонус по таблице KPI поиска
    kpi_pay: float = 0.0         # выплата «% от оплат» (KPI поиска)

    # KPI Влады (4 блока, ручной ввод, три отдельных поля для SMM)
    vlada_hr: float = 0.0        # БЛОК HR: бонусы за приём сотрудников (руб)
    vlada_smm: float = 0.0       # БЛОК SMM: оклад + бонус + клиенты (три поля) (руб)
    vlada_projects: float = 0.0  # БЛОК ПРОЕКТЫ: премии за сроки (финальная сумма) (руб)
    vlada_logistics: float = 0.0 # БЛОК ЛОГИСТИКА: бонус администрирования (финальное число) (руб)

    # KPI РОП (3 блока бонусов, планы вводит Анна, факты подтягиваются автоматически)
    rop_new_sales: float = 0.0   # БЛОК 2: бонус + премия за новые продажи (только блок 2 имеет премию) (руб)
    rop_turnover: float = 0.0    # БЛОК 3: бонус за оборот (без премии) (руб)
    rop_payments: float = 0.0    # БЛОК 4: бонус за поступления (без премии) (руб)
...
def compute_salary(shift_rows: List[list],
    ...
    kpi_inputs — разные структуры в зависимости от группы:
      • «поиск»: {сотрудник: {"plan_fact": оборот_по_новым, "payments": поступления}}
      • «vlada»: {сотрудник: {"hr": 0, "smm_salary": 25000, "smm_bonus": 0, "smm_clients": 0,
                              "projects": 0, "logistics": 10000}}  # TODO:уточнить
      • «rop»: {сотрудник: {"plan_new_sales": 0, "fact_new_sales": 0,
                            "plan_turnover": 0, "fact_turnover": 0,
                            "plan_payments": 0, "fact_payments": 0}}  # TODO:уточнить
    """
    ...
    # KPI считаем по группам
    for emp, r in agg.items():
        inp = kpi_inputs.get(emp, {})

        if r.group == "poisk":
            # KPI «поиска»: фикс-бонус + % от поступлений
            ...
            # KPI Влады (4 блока, вручную три отдельных числа для SMM)
            r.vlada_hr = float(inp.get("hr", 0) or 0)
            r.vlada_smm = (float(inp.get("smm_salary", 0) or 0) +
                           float(inp.get("smm_bonus", 0) or 0) +
                           float(inp.get("smm_clients", 0) or 0))
            r.vlada_projects = float(inp.get("projects", 0) or 0)
            r.vlada_logistics = float(inp.get("logistics", 0) or 0)

        elif r.group == "rop":
            # КПИ РОП (планы вводит Анна в свободной таблице, факты подтягиваются автоматически)
            # Премия 10k только для блока 2 (новые продажи)
            plan_new = float(inp.get("plan_new_sales", 0) or 0)
            fact_new = float(inp.get("fact_new_sales", 0) or 0)
            plan_turn = float(inp.get("plan_turnover", 0) or 0)
            fact_turn = float(inp.get("fact_turnover", 0) or 0)
            plan_pay = float(inp.get("plan_payments", 0) or 0)
            fact_pay = float(inp.get("fact_payments", 0) or 0)

            r.rop_new_sales = kpi_rop_block(plan_new, fact_new) + kpi_rop_premium(plan_new, fact_new)
            r.rop_turnover = kpi_rop_block(plan_turn, fact_turn)
            r.rop_payments = kpi_rop_block(plan_pay, fact_pay)

        # Итого = база + подработки + все KPI бонусы
        r.total = (r.base_pay + r.side_pay + r.kpi_fix + r.kpi_pay +
                   r.vlada_hr + r.vlada_smm + r.vlada_projects + r.vlada_logistics +
                   r.rop_new_sales + r.rop_turnover + r.rop_payments)
...
class SalaryModule:
    ...
    def _read_settings_plans(self) -> Dict[str, dict]:
        """Читает планы РОП и автоматические компоненты Влады из листа НАСТРОЙКИ (строки 13-19)"""
        try:
            ws = self.sheets.ws("НАСТРОЙКИ")
            if not ws:
                return {}
            data = ws.get_all_values()
            result = {}

            # Строки 13-15: планы РОП (индекс 12-14)
            for i in [12, 13, 14]:
                if i < len(data) and len(data[i]) >= 3:
                    if "РОП Кономенко" in data[i][0]:
                        indicator = data[i][1].strip()
                        try:
                            value = float(str(data[i][2]).replace(",", ".").replace(" ", ""))
                        except:
                            continue

                        if "Новые клиенты" in indicator:
                            result.setdefault("Анна Кономенко", {})["plan_new_sales"] = value
                        elif "Оплаты" in indicator and "Оборот" not in indicator:
                            result.setdefault("Анна Кономенко", {})["plan_payments"] = value
                        elif "Оборот" in indicator:
                            result.setdefault("Анна Кономенко", {})["plan_turnover"] = value

            # Строки 17-19: автоматические компоненты Влады (индекс 16-18)
            for i in [16, 17, 18]:
                if i < len(data) and len(data[i]) >= 3:
                    if "Владислава" in data[i][0]:
                        indicator = data[i][1].strip()
                        try:
                            value = float(str(data[i][2]).replace(",", ".").replace(" ", ""))
                        except:
                        ...
                        result.setdefault("Владислава Герасимчук", {})
                        if "HR оклад" in indicator:
                            result["Владислава Герасимчук"]["smm_salary_auto"] = value
                        elif "SMM оклад" in indicator:
                            result["Владислава Герасимчук"]["smm_salary_auto"] = value
                        elif "Логистика" in indicator:
                            result["Владислава Герасимчук"]["logistics_max"] = value

            return result
        except Exception as e:
            log.warning("Ошибка чтения НАСТРОЙКИ: %s", e)
            return {}

    def _read_rop_facts(self) -> Dict[str, dict]:
        """Читает факты РОП из листов НОВЫЕ_КЛИЕНТЫ и ДАННЫЕ"""
        try:
            result = {"Анна Кономенко": {}}

            # Факт новых продаж из листа НОВЫЕ_КЛИЕНТЫ
            ws = self.sheets.ws("НОВЫЕ_КЛИЕНТЫ")
            if ws:
                data = ws.get_all_values()
                # Ищем РОП в колонке C (Менеджер поиска / РОП), суммируем колонку G (Скорр. оплаты)
                total = 0.0
                for row in data[2:]:  # Пропускаем заголовки
                    if len(row) > 6 and len(row) > 2:
                        # В общем листе РОП считаются как менеджер "РОП" или содержит "Кономенко"
                        if "Кономенко" in str(row[2]) or "РОП" in str(row[2]):
                            try:
                                total += float(str(row[6]).replace(",", "."))
                            except:
                                pass
                result["Анна Кономенко"]["fact_new_sales"] = round(total)

            # Факты оборота и поступлений из листов ДАННЫЕ
            for sheet_name in ["ДАННЫЕ_Губарев", "ДАННЫЕ_Перфильев"]:
                ws = self.sheets.ws(sheet_name)
                if ws:
                    data = ws.get_all_values()
                    # Колонка I (индекс 8): Скорр. оплаты (поступления)
                    # Колонка K (индекс 10): Оборот
                    for row in data[2:]:
                        if len(row) > 10:
                            # Проверяем есть ли РОП в этой строке
                            # (в реальной таблице нужна более точная логика)
                            try:
                                payments = float(str(row[8]).replace(",", "."))
                                turnover = float(str(row[10]).replace(",", "."))
                                result["Анна Кономенко"]["fact_payments"] = result["Анна Кономенко"].get("fact_payments", 0) + payments
                                result["Анна Кономенко"]["fact_turnover"] = result["Анна Кономенко"].get("fact_turnover", 0) + turnover
                            except:
                                pass

            return result
        except Exception as e:
            log.warning("Ошибка чтения фактов РОП: %s", e)
            return {}

    def _kpi_inputs(self) -> Dict[str, dict]:
        """Объединяет КПИ из НАСТРОЙКИ и фактов из листов ДАННЫЕ"""
        settings = self._read_settings_plans()
        facts = self._read_rop_facts()

        # Объединяем (факты переопределяют планы)
        result = {**settings, **facts}
        return result
    ...
    def run(self, chat_id, period_label: str = ""):
        ...
        for r in rows:                                        # запись в СВОДНАЯ_ЗП
            # Для каждой группы своя структура вывода
            if r.group == "poisk":
                # Менеджеры поиска: как есть (из существующей структуры)
                row_data = [
                ...
                # Влада: компоненты вводятся вручную в листе (только структура)
                # Компоненты: Смены, Подр, HR оклад, HR найм, SMM оклад, SMM премия, Проекты, Логистика, Доп.премия
                row_data = [
                    period_label, r.employee, r.group, r.shifts,
                    round(r.base_pay), round(r.side_pay),
                    # Остальное вводится вручную в листе СВОДНАЯ_ЗП
                    # round(r.vlada_hr), round(r.vlada_smm), round(r.vlada_projects), round(r.vlada_logistics),
                    round(r.total),
                ]
            elif r.group == "rop":
                # РОП: три блока бонусов
                row_data = [
                    period_label, r.employee, r.group, r.shifts,
                    round(r.base_pay), round(r.side_pay),
                    round(r.rop_new_sales), round(r.rop_turnover), round(r.rop_payments),
                    round(r.total),
                ]
            else:  # soprovozhdenie или др.
                row_data = [
                    period_label, r.employee, r.group, r.shifts,
                    round(r.base_pay), round(r.side_pay),
                    round(r.total),
                ]
            self.sheets.append(SHEETS.SALARY, row_data)
        ...
        for r in rows:
            ...
            if r.group == "poisk":
                ...
                vlada_kpi = r.vlada_hr + r.vlada_smm + r.vlada_projects + r.vlada_logistics
                if vlada_kpi > 0:
                    extra = f", KPI +{vlada_kpi:,.0f}"
            elif r.group == "rop":
                rop_kpi = r.rop_new_sales + r.rop_turnover + r.rop_payments
                if rop_kpi > 0:
                    extra = f", KPI +{rop_kpi:,.0f}"
...
def selftest():
    ...
    print("\n== ТЕСТ KPI РОП (расчет одного блока и премия) ==")
    print("  Блок 2 (новые продажи): план 500k, факт 600k (120%)")
    block_bonus = kpi_rop_block(500000, 600000)
    premium = kpi_rop_premium(500000, 600000)
    print(f"    → бонус {block_bonus:.0f} ₽ + премия {premium:.0f} ₽ = {block_bonus + premium:.0f} ₽")

    print("  Блок 3 (оборот): план 2M, факт 1.5M (75%)")
    block_bonus = kpi_rop_block(2000000, 1500000)
    print(f"    → бонус {block_bonus:.0f} ₽ (0.7% от 1.5M при 75%)")

    print("  Блок 4 (поступления): план 800k, факт 500k (62.5%, < 70% → нет бонуса)")
    block_bonus = kpi_rop_block(800000, 500000)
    print(f"    → бонус {block_bonus:.0f} ₽")

    print("\n== ТЕСТ РАСЧЁТА ЗП ПО ГРУППАМ (с KPI) ==")
    ...
    kpi_poisk_input = {
        "Ксения Наныкина": {"plan_fact": 231000, "payments": 800000}
    }

    kpi_vlada_input = {
        "Владислава Герасимчук": {
            "hr": 20000,           # БЛОК HR: бонус за план найма
            "smm_salary": 25000,   # БЛОК SMM: оклад
            "smm_bonus": 5000,     # БЛОК SMM: премия 90%+ контента
            "smm_clients": 7000,   # БЛОК SMM: 1 клиент категории А
            "projects": 15000,     # БЛОК ПРОЕКТЫ: премия за сданный проект
            "logistics": 9500,     # БЛОК ЛОГИСТИКА: 10000 - 500 (одна ошибка)
        }
    }
    ...
    kpi_rop_input = {
        "Анна Кононенко": {
            ...
            "fact_new_sales": 720000,    # факт 120% → 7200 ₽ + 10000 ₽ премия
    ...
    kpi_all = {**kpi_poisk_input, **kpi_vlada_input, **kpi_rop_input}
    for r in compute_salary(shift_rows, kpi_all):
        print(f"  {r.employee} [{r.group}]:")
        print(f"    смен {r.shifts}, база {r.base_pay:.0f}, подр {r.side_pay:.0f}")
        if r.group == "poisk":
            print(f"    KPI поиск {r.plan_pct}% (+{r.kpi_fix:.0f}+{r.kpi_pay:.0f})")
        if r.group == "vlada":
            print(f"    KPI Влада: HR {r.vlada_hr:.0f} + SMM {r.vlada_smm:.0f} + "
                  f"проекты {r.vlada_projects:.0f} + логистика {r.vlada_logistics:.0f}")
        if r.group == "rop":
            print(f"    KPI РОП: новые {r.rop_new_sales:.0f} + оборот {r.rop_turnover:.0f} + "
                  f"поступления {r.rop_payments:.0f}")
        print(f"    ИТОГО {r.total:.0f} ₽\n")

    print("OK. Правила — в EMPLOYEE_GROUP / SHIFT_RATE / SIDE_RATE / KPI_TABLE_POISK / kpi_rop_block / kpi_rop_premium.")
